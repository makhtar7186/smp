"""Vue Archive : archive/désarchive un lot de factures usine sélectionnées.

Une facture archivée disparaît de l'historique des ventes et des tableaux de
bord (dashboard, analyse produits) ; elle reste intacte pour les factures
clients et le suivi des marges, qui ne filtrent jamais sur `archivee`.
"""
...
class ArchiveView(ttk.Frame):
    """Sélection multiple de factures usine à archiver, ou à réafficher."""
    ...
    def _construire(self) -> None:
        ...
        # Filtres + mode d'affichage (actives / archivées)
        ...
        tk.Label(filtres, text=t("du"), bg=theme.COULEURS["carte"]).pack(
            side="left", padx=(theme.PAD, 4))
        # Champs de date vidés après construction (DateEntry pré-remplit sinon
        # avec la date du jour) — un filtre Du/Au vide signifie « aucune borne ».
        ...
        self._table.grid(row=3, column=0, sticky="nsew", padx=theme.PAD_L,
                         pady=theme.PAD)
        ...
        barre.grid(row=4, column=0, sticky="ew", padx=theme.PAD_L,
                  pady=(0, theme.PAD_L))
    ...
    def rafraichir(self) -> None:
        """Recharge la liste (actives ou archivées selon le mode choisi)."""
        ...
        toutes = self.app.facturation.lister_factures(
            date_debut, date_fin, client_id, inclure_archivees=True)
        ...
        for facture in self._factures:
            self._table.ajouter([
                ...
                facture.nb_lignes, format_fcfa(facture.total_liste, False),
            ])
    ...
    def _traiter_selection(self) -> None:
        ...
        if not iids:
            ...
            return
        ids_factures = [self._factures[self._table.tree.index(iid)].id
                        for iid in iids]

        if self._mode_archivees():
            nb = self.app.facturation.desarchiver_ids(ids_factures)
            ...
            if not messagebox.askyesno(
                ...
                return
            nb = self.app.facturation.archiver_ids(ids_factures)
